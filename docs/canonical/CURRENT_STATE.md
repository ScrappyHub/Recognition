...
| Recovery Engine / Portable Backup (§21, Deterministic Backup) | DONE: `recognition_backup_v1` (export/verify/import) produces a single portable, passphrase-encrypted bundle (AES-256-GCM, PBKDF2-SHA256 600k iterations — the same crypto core primitives as the Vault, chosen over DPAPI specifically because DPAPI ciphertext is machine/account-bound and would make a "portable" backup a false claim) carrying the identity (including its DPAPI-sealed secret, re-sealed on the target machine after restore) and every governed runtime store (history/bookmarks/downloads/actions/cookies); restoring preserves the exact `recognition_identity_id`; refuses to overwrite a different existing identity or an existing store without `-Force`; a store absent at export time is never fabricated on restore; negative vectors: wrong passphrase, tampered ciphertext, both refusal paths | `_selftest_recognition_backup_v1` | `SELFTEST_RECOGNITION_BACKUP_V1_OK` |
...
| Encrypted Cookies (§23) | DONE (browser-level): cookie VALUES are already OS-encrypted at rest by the engine's own profile store; Recognition adds its own governed witness — a deterministic, append-only, hash-chained, **DPAPI-encrypted** ledger (`runtime\cookies.v1.enc`) of every cookie add/change (deduped to real changes) and every clear action, with cookie name+value stored only as a combined SHA-256 (never cleartext); domain kept in the action label (already visible elsewhere, e.g. downloads/bookmarks); "Clear cookies for this site" / "Clear all cookies" in Settings; surfaced live + `cookie_receipts.json` in every exported packet; negative-vector selftest in prove-all | `_selftest_recognition_cookie_receipts_v1` | `SELFTEST_RECOGNITION_COOKIE_RECEIPTS_V1_OK` |
...
| Identity Vault / Layer 0 (§9,§27) | DONE: the identity descriptor (`identity.json`) holds only non-secret, one-way hash outputs (device_id/user_id/vault_id/recognition_identity_id); the one reproducible secret — the salt — is sealed at rest via **Windows DPAPI** (CurrentUser scope, no passphrase, same model as history/actions/cookies/downloads/bookmarks) into `identity.salt.enc`, never written in cleartext; legacy descriptors with an inline salt are migrated (sealed + redacted) on next `RID-EnsureIdentity`; negative vectors: missing sealed file and tampered ciphertext both fail closed | `_selftest_recognition_identity_vault_v1` | `SELFTEST_RECOGNITION_IDENTITY_VAULT_V1_OK` |
...
| Deterministic Backup | DONE — see "Recovery Engine / Portable Backup (§21)" above; freeze bundles + Vault remain the separate evidence-sealing path, not superseded | `_selftest_recognition_backup_v1` | `SELFTEST_RECOGNITION_BACKUP_V1_OK` |
