...
function ParseGateFile([string]$Path){ $t=$null;$e=$null; $null=[System.Management.Automation.Language.Parser]::ParseFile($Path,[ref]$t,[ref]$e); if($e -and $e.Count -gt 0){ $x=$e[0]; Die ("PARSE_GATE_FAIL: {0}:{1}:{2}: {3}" -f $Path,$x.Extent.StartLineNumber,$x.Extent.StartColumnNumber,$x.Message) } }
...
[void]$L.Add("function PC-ReadAllBytes([string]$Path){ if(-not (Test-Path -LiteralPath $Path -PathType Leaf)){ PC-Die (""MISSING_FILE: "" + $Path) }; return [System.IO.File]::ReadAllBytes($Path) }")
[void]$L.Add("function PC-Sha256HexBytes([byte[]]$Bytes){ if($null -eq $Bytes){ PC-Die ""PC-Sha256HexBytes: null bytes"" }; $sha=[System.Security.Cryptography.SHA256]::Create(); try{ $h=$sha.ComputeHash($Bytes) } finally{ $sha.Dispose() }; $sb=New-Object System.Text.StringBuilder; foreach($b in $h){ [void]$sb.Append($b.ToString(""x2"")) }; return $sb.ToString() }")
...
[void]$L.Add("    $ch = $s[$i]")
[void]$L.Add("    switch ($ch) {")
[void]$L.Add("      ""`b"" { [void]$sb.Append(""\\b""); continue }")
[void]$L.Add("      ""`t"" { [void]$sb.Append(""\\t""); continue }")
[void]$L.Add("      ""`n"" { [void]$sb.Append(""\\n""); continue }")
[void]$L.Add("      ""`f"" { [void]$sb.Append(""\\f""); continue }")
[void]$L.Add("      ""`r"" { [void]$sb.Append(""\\r""); continue }")
[void]$L.Add("      """"  { [void]$sb.Append(""\\\""" ); continue }")
[void]$L.Add("      ""\\"" { [void]$sb.Append(""\\\\"" ); continue }")
[void]$L.Add("      default {")
[void]$L.Add("        $code = [int][char]$ch")
[void]$L.Add("        if($code -lt 32){ [void]$sb.Append(( '\u{0:x4}' -f $code )); continue }")
[void]$L.Add("        [void]$sb.Append($ch)")
[void]$L.Add("      }")
[void]$L.Add("    }")
[void]$L.Add("  }")
...
[void]$L.Add("    foreach($k in $keys){ $kk=[string]$k; $vv=$v[$kk]; [void]$parts.Add( (""\""""+(PC-EscapeJsonString $kk)+""\""":""+(PC-ToCanonJson $vv)) ) }")
...
[void]$L.Add("function PC-ComputePacketIdFromManifestNoIdCanon([string]$manifestCanonNoId){ $b=PC-CanonBytesFromCanonJson $manifestCanonNoId; return (PC-Sha256HexBytes $b) }")
[void]$L.Add("")
...
# Run selftest once as part of install
...
Write-Host ("PATCH_OK: PC v1 Option A installed + selftested; exporter ready -> " + $expPath) -ForegroundColor Green
