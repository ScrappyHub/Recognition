...
EnsureDir (Join-Path $RepoRoot "test_vectors")
...
[void]$L.Add("function PC-Sha256HexFile([string]$Path){ $sha=[System.Security.Cryptography.SHA256]::Create(); try{ $fs=[System.IO.File]::OpenRead($Path); try{ $h=$sha.ComputeHash($fs) } finally{ $fs.Dispose() } } finally{ $sha.Dispose() }; $sb=New-Object System.Text.StringBuilder; foreach($b in $h){ [void]$sb.Append($b.ToString(""x2"")) }; return $sb.ToString() }")
...
[void]$L.Add("    if($code -lt 32){ [void]$sb.Append(""\\u""); [void]$sb.Append($code.ToString(""x4"")); continue }")
...
[void]$L.Add("  if($v -is [int] -or $v -is [long] -or $v -is [double] -or $v -is [decimal]){ return ([string]::Format([System.Globalization.CultureInfo]::InvariantCulture,""{0}"",$v)).ToLowerInvariant() }")
...
Write-Host ("PATCH_OK: Packet Constitution v1 Option A installed + selftested (standalone). Exporter ready -> " + $expPath) -ForegroundColor Green
