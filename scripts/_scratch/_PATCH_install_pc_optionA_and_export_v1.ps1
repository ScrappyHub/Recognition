...
EnsureDir (Join-Path $RepoRoot "payload/session_export")
EnsureDir (Join-Path $RepoRoot "test_vectors/packet_constitution_v1/v1_minimal_optionA")
...
[void]$L.Add("function PC-Sha256HexBytes([byte[]]$Bytes){ if($null -eq $Bytes){ PC-Die ""PC-Sha256HexBytes: null bytes"" }; $sha=[System.Security.Cryptography.SHA256]::Create(); try{ $h=$sha.ComputeHash($Bytes) } finally{ $sha.Dispose() }; $sb=New-Object System.Text.StringBuilder; foreach($b in $h){ [void]$sb.Append($b.ToString(""x2"")) }; $sb.ToString() }")
[void]$L.Add("function PC-Sha256HexFile([string]$Path){ if(-not (Test-Path -LiteralPath $Path -PathType Leaf)){ PC-Die (""MISSING_FILE: "" + $Path) }; $sha=[System.Security.Cryptography.SHA256]::Create(); try{ $fs=[System.IO.File]::OpenRead($Path); try{ $h=$sha.ComputeHash($fs) } finally{ $fs.Dispose() } } finally{ $sha.Dispose() }; $sb=New-Object System.Text.StringBuilder; foreach($b in $h){ [void]$sb.Append($b.ToString(""x2"")) }; $sb.ToString() }")
[void]$L.Add("function PC-NowUtc(){ (Get-Date).ToUniversalTime().ToString(""yyyy-MM-ddTHH:mm:ss.fffZ"") }")
[void]$L.Add("")
[void]$L.Add("function PC-EscapeJsonString([string]$s){ if($null -eq $s){ return """" }; $sb=New-Object System.Text.StringBuilder; for($i=0;$i -lt $s.Length;$i++){ $c=[int][char]$s[$i]; switch($c){ 8{[void]$sb.Append(""\\b"");continue} 9{[void]$sb.Append(""\\t"");continue} 10{[void]$sb.Append(""\\n"");continue} 12{[void]$sb.Append(""\\f"");continue} 13{[void]$sb.Append(""\\r"");continue} 34{[void]$sb.Append(""\\\""");continue} 92{[void]$sb.Append(""\\\\"");continue} default{ if($c -lt 32){ [void]$sb.Append((""\u{0:x4}"" -f $c)); continue }; [void]$sb.Append([char]$c) } } }; $sb.ToString() }")
[void]$L.Add("")
...
[void]$L.Add("  if($v -is [bool]){ if($v){ return ""true"" } else { return ""false"" } }")
[void]$L.Add("  if($v -is [int] -or $v -is [long] -or $v -is [decimal]){ return ([string]::Format([System.Globalization.CultureInfo]::InvariantCulture,""{0}"",$v)) }")
[void]$L.Add("  if($v -is [double]){ return ([string]::Format([System.Globalization.CultureInfo]::InvariantCulture,""{0:R}"",$v)) }")
...
[void]$L.Add("    foreach($kk in $keys){ $vv = $v[$kk]; [void]$parts.Add( ("""""" + (PC-EscapeJsonString $kk) + """""":"" + (PC-ToCanonJson $vv)) ) }")
...
[void]$L.Add("}")
[void]$L.Add("")
[void]$L.Add("function PC-CanonBytesFromCanonJson([string]$canon){ $t=($canon -replace ""`r`n"",""`n"") -replace ""`r"",""`n""; if(-not $t.EndsWith(""`n"")){ $t += ""`n"" }; (New-Object System.Text.UTF8Encoding($false)).GetBytes($t) }")
[void]$L.Add("function PC-ComputePacketIdFromManifestNoIdCanon([string]$manifestCanonNoId){ PC-Sha256HexBytes (PC-CanonBytesFromCanonJson $manifestCanonNoId) }")
[void]$L.Add("function PC-RelPath([string]$Root,[string]$Full){ $r=[System.IO.Path]::GetFullPath($Root); $f=[System.IO.Path]::GetFullPath($Full); if(-not $f.StartsWith($r,[System.StringComparison]::OrdinalIgnoreCase)){ PC-Die (""PC-RelPath: not under root: "" + $Full) }; $rel=$f.Substring($r.Length).TrimStart(""\"",""/""); ($rel -replace ""\"",""/"") }")
[void]$L.Add("function PC-ListFilesRec([string]$Dir){ if(-not (Test-Path -LiteralPath $Dir -PathType Container)){ PC-Die (""MISSING_DIR: "" + $Dir) }; @(@(Get-ChildItem -LiteralPath $Dir -Recurse -File -Force | Sort-Object FullName)) }")
...
Write-Host ("PATCH_OK: Packet Constitution v1 Option A installed + selftested; export script ready -> " + (Join-Path $ScriptsDir "recognition_export_session_packet_v1.ps1")) -ForegroundColor Green
