...
# =========================================================
# scripts/_lib_packet_constitution_v1.ps1  (standalone)
# - Parse-safe JSON escaping (char-code appends; no \" hazards)
# - Canon JSON stable ordering, UTF-8 no BOM + LF
# - NO regex path operations (use .Replace / char codes)
# =========================================================
...
[void]$L.Add('    if($code -eq 8){  [void]$sb.Append([char]92); [void]$sb.Append([char]98);  continue }')   # \b
[void]$L.Add('    if($code -eq 9){  [void]$sb.Append([char]92); [void]$sb.Append([char]116); continue }')   # \t
[void]$L.Add('    if($code -eq 10){ [void]$sb.Append([char]92); [void]$sb.Append([char]110); continue }')   # \n
[void]$L.Add('    if($code -eq 12){ [void]$sb.Append([char]92); [void]$sb.Append([char]102); continue }')   # \f
[void]$L.Add('    if($code -eq 13){ [void]$sb.Append([char]92); [void]$sb.Append([char]114); continue }')   # \r
[void]$L.Add('    if($code -eq 34){ [void]$sb.Append([char]92); [void]$sb.Append([char]34);  continue }')   # \"
[void]$L.Add('    if($code -eq 92){ [void]$sb.Append([char]92); [void]$sb.Append([char]92);  continue }')   # \\
[void]$L.Add('    if($code -lt 32){')
[void]$L.Add('      [void]$sb.Append([char]92)')    # \
[void]$L.Add('      [void]$sb.Append([char]117)')   # u
[void]$L.Add('      [void]$sb.Append(("{0:x4}" -f $code))')
[void]$L.Add('      continue')
[void]$L.Add('    }')
...
# - deterministic PacketId (manifest has NO timestamps)
# - Copy-Item uses -Recurse only for directories
# =========================================================
...
[void]$B.Add('    Copy-Item -LiteralPath $c.FullName -Destination $pktPayload -Recurse -Force')
[void]$B.Add('  } else {')
[void]$B.Add('    Copy-Item -LiteralPath $c.FullName -Destination $pktPayload -Force')
[void]$B.Add('  }')
[void]$B.Add('}')
[void]$B.Add('')
...
# =========================================================
# Run selftest then exporter (must pass or throw)
# =========================================================
& $selfPath -RepoRoot $RepoRoot | Out-Host
& $expPath  -RepoRoot $RepoRoot | Out-Host

Write-Host ("PC_V6_OK: lib/builder/verifier/selftest/exporter are GREEN -> " + $ScriptsDir) -ForegroundColor Green
