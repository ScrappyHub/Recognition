...
function FailIfBadReplace([string]$Path){
  # Detect the specific footgun: -replace "\" (invalid regex pattern "\")
...
if([string]::IsNullOrWhiteSpace($RepoRoot)){ Die "RepoRoot is empty" }
if(-not (Test-Path -LiteralPath $RepoRoot -PathType Container)){ Die ("MISSING_REPOROOT_DIR: " + $RepoRoot) }
...
[void]$L.Add('}')
[void]$L.Add('')
[void]$L.Add('# Canonical JSON (stable ordering, stable escaping, no whitespace)')
...
# =========================================================
# scripts/_selftest_packet_constitution_v1.ps1  (in-process, throws)
# =========================================================
...
# =========================================================
# Run selftest (MUST PASS) then run exporter (MUST PASS)
# =========================================================
& $selfPath -RepoRoot $RepoRoot | Out-Host
& $expPath  -RepoRoot $RepoRoot | Out-Host

Write-Host ("PC_V5_OK: lib/builder/verifier/selftest/exporter are GREEN -> " + $ScriptsDir) -ForegroundColor Green
