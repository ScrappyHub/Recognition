...
function Copy-Dir([string]$Src,[string]$Dst){
  if(-not (Test-Path -LiteralPath $Src -PathType Container)){ Die ("COPYDIR_MISSING_SRC: " + $Src) }
...
# NEG 1: packet_id tamper should fail on sha256sums first
$pkt1 = Join-Path $work "tampered_packet_id"
Copy-Dir $goodPkt $pkt1
WriteUtf8NoBomLf (Join-Path $pkt1 "packet_id.txt") ("0" * 64)
Expect-FailToken { & $Verifier -PacketDir $pkt1 } "SHA256_MISMATCH: packet_id.txt" "NEG_PACKET_ID"

# NEG 2: manifest tamper should fail on manifest sha mismatch
...
# NEG 3: payload tamper should fail on payload sha mismatch
...
Write-Host "RECOGNITION_PC_NEGATIVE_VECTORS_V3_OK" -ForegroundColor Green
